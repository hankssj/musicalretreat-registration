class LoginController < ApplicationController
  ...
  def login
    ...
    if request.post? && params[:email]
      ...
      if (!user)
        ...
        redirect_to(session[:original_uri] || {:controller => :registration, :action => :index})
      end
    end
  end
  ...
  #  This is from the registration page, by the user.  Requires authentication on old password
  def change_password
    if request.post? && params[:email]
  ...
  # This is from admin page.  Do not need old password.  Have email, new_password, and confirm_new_password
  def reset_password
    if request.post?
  ...
  def change_email
    ...
    if request.post?
      ...
      if !user
        ...
        flash[:notice] = "Cannot find email address #{old_email}-- do you want to create a new user instead?"
  ...
  # From registration -- generate new password and send email
