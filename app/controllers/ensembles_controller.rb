class EnsemblesController < ApplicationController
  ...
  def chamber
    ...
    [1..num_mmr].each{|i| MmrChamber.create!(:ensemble_primary_id => @ensemble_primary.id)}
    [1..num_prearranged].each{|i| PrearrangedChamber.create!(:ensemble_primary_id => @ensemble_primary.id)}
