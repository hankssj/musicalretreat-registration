class EnsemblesController < ApplicationController
  ...
  def chamber
    ...
    [1.num_mmr].each{|i| MmrChamber.create!(:primary_ensemble_id = @ensemble_primary.id)}
    [1.num_prearranged].each{|i| PrearrangedChamber.create!(:primary_ensemble_id = @ensemble_primary.id)}
